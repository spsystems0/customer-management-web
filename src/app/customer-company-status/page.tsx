...
function formatTransactionStartMonth(value?: string | null) {
  if (!value) return ''
...
export default function CustomerCompanyStatusPage() {
  ...
  const [appliedCategoryCode, setAppliedCategoryCode] = useState('all')
  ...
  function getCategoryLabel(code?: string | null) {
    ...
    const category = categoryCodeMap.get(code)

    if (!category) return code

    return category.code_name
  }
  ...
  const displayRows = useMemo<DisplayRow[]>(() => {
    ...
    if (appliedCategoryCode !== 'all') {
      result = result.filter(
        (company) => company.customer_category_code === appliedCategoryCode
      )
    }
    ...
    return result
      .map((company) => ({
        ...
        customerCategoryCode: getCategoryLabel(company.customer_category_code),
        transactionStartMonth: formatTransactionStartMonth(
          company.transaction_start_date
        ),
  ...
  }, [companies, appliedCategoryCode, hasSearched, categoryCodeMap])

  function handleSearch() {
    setMessage('')
    setAppliedCategoryCode(selectedCategoryCode)
    setHasSearched(true)
  }
  ...
  function handleExcelDownload() {
    ...
    const selectedCategoryLabel =
      appliedCategoryCode === 'all'
        ? '전체'
        : getCategoryLabel(appliedCategoryCode)
      ...
      .map(
        (row) => `
          <tr>
            ...
            <td>${escapeHtml(row.customerCategoryCode)}</td>
            <td class="text-cell">${escapeHtml(row.transactionStartMonth)}</td>
        ...
        <body>
          <table>
            ...
            <tr>
              ...
              <td colspan="6">${escapeHtml(selectedCategoryLabel)}</td>
            </tr>
            ...
            <tr>
              ...
              <th class="header">거래시작월</th>
              <th class="header">업종</th>
  ...
  function handleRowClick(row: DisplayRow) {
    ...
    params.set('companyId', String(row.id))
          ...
          <div className="mx-auto max-w-7xl space-y-6">
            <div className="rounded-2xl bg-white p-8 shadow">
              ...
              <p className="mt-2 text-slate-600">
                고객분류코드 조건으로 고객사 현황을 조회하고 Excel로 다운로드할 수 있습니다.
              </p>
            </div>
            ...
            <div className="rounded-2xl bg-white p-8 shadow">
              ...
              {hasSearched && displayRows.length > 0 && (
                <p className="mt-3 text-sm text-slate-500">
                  고객사를 클릭하면 고객사관리카드 조회 화면으로 이동합니다.
                </p>
              )}
            </div>
          </div>
